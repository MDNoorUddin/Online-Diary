...
$date=$_GET['date'];
$result = mysqli_query($con,"SELECT title,paragraph FROM `article` WHERE email_id = '$email' and  date='$date'") or die('Error');
...
<?php
    if(isset($_POST['submit']))
    {
        ...
        $email=$_SESSION['email'];
        $d=$_GET['date'];
        $q3=mysqli_query($con,"INSERT INTO article (email_id,date,title,paragraph) VALUES  ('$email','$d', '$title','$note')");
        if($q3)
        {
...
<form style =" position:absolute;" action="" method="post" style="border:1px solid #ccc">
  <div class="container">
    ...
    <textarea style="width:400px;position:absolute;height:300px;" placeholder="Write Note" name="note"></textarea>
